/* ---------------------------------------------------------------------
 * NODO ESCLAVO - ULTRASONIDO (SRF02)
 * ---------------------------------------------------------------------
 */
#include <SPI.h>
#include <LoRa.h>
...
const uint8_t localAddress = SENSOR_ID_ULTRA;
...
void loop()
{
	if (!transmitting && (millis() - lastSensorReadTime) >= SENSOR_READ_INTERVAL)
	{
		...
		uint8_t state;
		if (distInt < 0)
			state = DIST_STATE_ERROR;
		else if (distInt < 30)
			state = DIST_STATE_CLOSE;
		else if (distInt < 100)
			state = DIST_STATE_MEDIUM;
		else
			state = DIST_STATE_FAR;
