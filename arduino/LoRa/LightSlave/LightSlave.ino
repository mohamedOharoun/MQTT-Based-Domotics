...
#define SENSOR_ID_LIGHT 0x02  // IDENTIFICADOR ÚNICO DEL SENSOR
...
void setup() {
  ...
  // ERROR SOURCE: This line pauses the code forever if the Serial Monitor isn't open.
  // while (!Serial);  <-- REMOVE THIS
  
  // Give the serial port a moment to stabilize, then print immediately
  delay(2000); 
  Serial.println("\n=== INICIANDO SISTEMA ===");
  ...
  // Configuración LoRa matches Master
  ...
  // FORCE IMMEDIATE SEND: Trick the timer to think 5 seconds have passed
  lastSendTime = millis() - TX_LAPSE_MS; 
}

void loop() {
  // Logic remains the same, but now lastSendTime is pre-set to trigger immediately
  if (!transmitting && (millis() - lastSendTime) > TX_LAPSE_MS) {
    lastSendTime = millis();
    
    float luxVal = light.readLight();
    
    // Debug: Print reading to confirm sensor is working
    ...
    uint8_t state;
    if (luxVal < 200) state = LIGHT_STATE_DARK;
    else if (luxVal < 800) state = LIGHT_STATE_DIM;
    else state = LIGHT_STATE_BRIGHT;
