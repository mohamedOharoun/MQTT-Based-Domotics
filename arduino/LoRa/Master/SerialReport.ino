...
#define ADDR_ULTRASONIC 0xBB
#define ADDR_LIGHT      0xCC
...
String serialbridge_node_hex_to_string(uint8_t node_hex)
{
  ...
  if (node_hex == ADDR_ULTRASONIC) {
    node_type = "ULTRASONIC_";
  } 
  else if (node_hex == ADDR_LIGHT) {
    node_type = "LIGHT_";
  } 
  else {
    node_type = "UNKNOWN_";
...
{
  if (node_hex == ADDR_LIGHT) return "light";
  if (node_hex == ADDR_ULTRASONIC) return "ultrasonic";
  return "unknown";
}
...
void serialbridge_report_event_trigger(uint8_t node_hex_id, EventType_t *event)
{
  ...
  String trigger_type_str = (event->trigger_type == EventTriggerType::TRIGGER_ABOVE) ? "above" : 
                            (event->trigger_type == EventTriggerType::TRIGGER_BELOW) ? "below" : "equal";
